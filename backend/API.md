# Dark Pool Service: Engineering Guide and API Reference
...
**Change the interval N:** `PUT /venue/schedule { intervalMs }` (§6), 1 s to 24 h. Re-arms immediately; **not persisted** (reverts to `MATCH_INTERVAL_MS` on restart).
