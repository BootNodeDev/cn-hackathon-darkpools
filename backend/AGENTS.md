...
| `CANTON_JSON_API_URL` | yes | (none) | Canton JSON Ledger API v2 endpoint |
| `DARK_POOL_BOOTSTRAP` | yes | (none) | Path to `dark-pool.bootstrap.json` |
| `CANTON_BACKEND_TOKEN` | yes (static) | (none) | Static bearer JWT |
| `FIVENORTH_CLIENT_SECRET` | yes (M2M) | (none) | M2M OAuth secret (overrides static token) |
...
| `MATCH_INTERVAL_MS` | no | `300000` | Heartbeat interval (1s to 24h) |
